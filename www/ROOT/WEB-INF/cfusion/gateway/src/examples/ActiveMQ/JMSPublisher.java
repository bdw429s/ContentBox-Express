...
public class JMSPublisher
{
    ...
    void sendMessage(Map data) throws Exception
    {
        ...
        JMSPublishConnection publisher = getPublishConnection(isTopic, destinationName);
    ...
    private JMSPublishConnection getPublishConnection(boolean isTopic, String destinationName) throws Exception
    {
        JMSPublishConnection publisher = null;
        String shortName = null;
        if (this.useCache)
        {
            shortName = JMSConfiguration.getShortDestinationName(destinationName);
            publisher = (JMSPublishConnection) this.cache.get(shortName);
        }
        if (publisher == null)
        {
            publisher = new JMSPublishConnection(this.configuration, this.gateway, destinationName, isTopic);
            if (this.useCache)
            {
